...
1. **浅色模式清爽呈现**：在 `.dsh-canvas-light` 作用域下，边线彻底移除或禁用发光阴影（`filter: none`），改以高对比度单色实线或深色低对比虚线呈现，杜绝白底晕轮。
2. **深色模式受限发光**：深色模式下的 `drop-shadow` 模糊半径严格限制在 `<= 3px`，透明度 `<= 0.45`，消除刺眼赛博朋克光晕，回归工程工具本色（ADR-0009）。
...
2. **类名完全解耦**：实线脉冲类名与流动虚线类名严格互斥，杜绝 CSS 属性竞争。
