...
export class AdmissionLease {
   ...
   */
  constructor({ safeguard, callerSessionId, workspace, title, generation, timestamp }) {
  ...
  commit() {
    ...
    if (this.safeguard) {
      this.safeguard._releaseInFlight(this.workspace, this.title);
    }
    return this;
  }
...
export class SessionSafeguard {
  ...
  getInFlightTitles(workspace) {
    const ws = workspace ? normalizeWorkspace(workspace) : '';
    return this.inFlightByWorkspace.get(ws) || new Set();
  }
  ...
  acquire(params = {}) {
    ...
    if (title) {
      ...
      inFlightSet.add(title.toLowerCase());
    }
