...
export class AdmissionLease {
   ...
   */
  constructor({ safeguard, leaseId, callerSessionId, workspace, title, generation, timestamp }) {
  ...
  commit() {
    ...
    if (this.safeguard) {
      this.safeguard._releaseInFlight(this.workspace, this.leaseId, this.title);
    }
    return this;
  }
...
export class SessionSafeguard {
  ...
  constructor(options = {}) {
    ...
    this.inFlightTitlesByWorkspace = new Map();
    this.inFlightLeasesByWorkspace = new Map();
  ...
  reset() {
    ...
    this.inFlightTitlesByWorkspace.clear();
    this.inFlightLeasesByWorkspace.clear();
  }
  ...
  getInFlightCount(workspace) {
    ...
    const set = this.inFlightLeasesByWorkspace.get(ws);
  ...
  getInFlightTitles(workspace) {
    const ws = workspace ? normalizeWorkspace(workspace) : '';
    return this.inFlightTitlesByWorkspace.get(ws) || new Set();
  }

  _registerInFlight(workspace, leaseId, title) {
    const ws = workspace ? normalizeWorkspace(workspace) : '';
    let leaseSet = this.inFlightLeasesByWorkspace.get(ws);
    if (!leaseSet) {
      leaseSet = new Set();
      this.inFlightLeasesByWorkspace.set(ws, leaseSet);
    }
    leaseSet.add(leaseId);

    if (title && typeof title === 'string') {
      let titleSet = this.inFlightTitlesByWorkspace.get(ws);
      if (!titleSet) {
        titleSet = new Set();
        this.inFlightTitlesByWorkspace.set(ws, titleSet);
      }
      titleSet.add(title.toLowerCase());
    }
  }

  _releaseInFlight(workspace, leaseId, title) {
    const ws = workspace ? normalizeWorkspace(workspace) : '';
    const leaseSet = this.inFlightLeasesByWorkspace.get(ws);
    if (leaseSet && leaseId) {
      leaseSet.delete(leaseId);
      if (leaseSet.size === 0) {
        this.inFlightLeasesByWorkspace.delete(ws);
      }
    }
    if (title && typeof title === 'string') {
      const titleSet = this.inFlightTitlesByWorkspace.get(ws);
      if (titleSet) {
        titleSet.delete(title.toLowerCase());
        if (titleSet.size === 0) {
          this.inFlightTitlesByWorkspace.delete(ws);
        }
      }
    }
  }
  ...
  acquire(params = {}) {
    ...
    const metadata = callerAgent?.session?.metadata;
    ...
    // 6. 在途并发与标题登记（通过 leaseId 与标题集合正交隔离）
    const leaseId = typeof randomUUID === 'function'
      ? `lease-${randomUUID()}`
      : `lease-${Math.random().toString(36).slice(2, 10)}`;

    this._registerInFlight(normWorkspace, leaseId, title);
