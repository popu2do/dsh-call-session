...
export function inspectWorkspaceActiveSessions(agentsService, callerWorkspace, archivedIds, ctx) {
  ...
  for (const agent of liveAgents) {
    ...
    if (agent.origin === 'subagent' || agent.blank) continue;
 ...
 * @param {any} [rawOptions] 扩展参数（例如 boardStore）
...
export async function executeSessionCreate(ctxOrOptions, rawArgs = {}, rawExec = {}, rawOptions = {}) {
  ...
  if (callerGen >= PEER_SESSION_CONSTANTS.MAX_GENERATION) {
    throw new Error(
      `[GenerationLimitExceeded] 当前会话代际深度为 ${callerGen}，已达上限 ${PEER_SESSION_CONSTANTS.MAX_GENERATION}，禁止进一步递归派生平级会话。`
    );
  }
  ...
  if (activeCount >= PEER_SESSION_CONSTANTS.MAX_ACTIVE_PEER_SESSIONS) {
    throw new Error(
      `[QuotaExceeded] 当前工作区活跃同级会话数已达上限 ${PEER_SESSION_CONSTANTS.MAX_ACTIVE_PEER_SESSIONS}，拒绝创建新会话。`
    );
  }

  // 5. Title 安全过滤、推导与防重名校验
  ...
  // 6. 调用 ctx.root.agents.create 创建解耦平级 Root Session
  ...
  if (targetAgent) {
    ...
    if (targetAgent.session) {
      ...
      targetAgent.session.header = targetAgent.session.header || {};
  ...
  // 6.1 自动挂载当前工作区，确保在 Web GUI 归属当前工程，避免落入未分组
  ...
  if (rawInitialMessage) {
    const cleanPostIds = Array.isArray(args.context_post_ids)
      ? args.context_post_ids.map(String).map(s => s.trim()).filter(Boolean).slice(0, PEER_SESSION_CONSTANTS.MAX_CONTEXT_POST_IDS)
      : [];
