...
export function getReservedTopicErrorMessage(topic) {
  return `保留主题拦截: 主题 "${topic}" 属于纯内存遥测域 (ADR-0012)，严禁写入持久化黑板。`;
}
 ...
 */
export function formatAuthorReminderText(posts) {
  ...
  const countStr = count > maxDisplay ? ` 等共 ${count} 条` : '';
  return `你在公共黑板上有 ${count} 条尚未清理的有效条目（${items}${countStr}），任务完成后请调用 board_clear 及时清理。`;
}
...
export class BoardStore {
      ...
      for (const entry of entries) {
        if (entry.startsWith(prefix)) {
          try {
            ...
            this.logger.debug?.(`[BoardStore] 已清理异常退出残留临时文件: ${entry}`);
          } catch {}
        }
      }
    } catch {}
  ...
  recoverFromBackup() {
    if (!existsSync(this.backupPath)) {
      this.logger.warn?.('[BoardStore] 无可用备份文件，初始化空黑板。');
      return;
    }
    try {
      ...
      this.logger.debug?.('[BoardStore] 成功从 board.json.bak 恢复黑板数据');
      this.scheduleFlush();
    } catch (bakErr) {
      this.logger.warn?.('[BoardStore] 备份文件损坏，初始化空黑板:', bakErr.message);
    }
  }
  ...
  hydratePosts(data) {
    ...
    const now = Date.now();
    let cleanedDirtyCount = 0;
    for (const post of data.posts) {
      if (!post || !post.id) continue;
      if (isReservedTopic(post.topic)) {
        cleanedDirtyCount++;
        this.logger.warn?.(`[BoardStore] 已清洗误写入的保留主题脏数据: #${post.id} (topic: ${post.topic})`);
        continue;
      }
    ...
    if (cleanedDirtyCount > 0) {
      this.logger.info?.(`[BoardStore] 启动水合完成：已过滤清理 ${cleanedDirtyCount} 条包含保留主题或内部轨迹的脏数据`);
      this.scheduleFlush();
    }
    return cleanedDirtyCount;
  }
  ...
  post(record) {
    if (!record || typeof record !== 'object') {
      throw new Error('board_post 参数必须是非空对象。');
    }
    if (isReservedTopic(record.topic)) {
      throw new Error(getReservedTopicErrorMessage(record.topic));
    }
  ...
  clear({ id, topic, action = 'archive', callerWorkspace } = {}) {
    if (!id && !topic) {
      throw new Error('board_clear 必须指定 id 或 topic 筛选条件。');
    }
      ...
      if (post) {
        ...
        if (effectiveCallerWs && postWs && postWs !== effectiveCallerWs) {
          return {
            ...
            affectedCount: 0,
            action: cleanAction,
            message: `已${cleanAction === 'delete' ? '删除' : '归档'} 0 条黑板条目。`
          };
        }
    ...
    return {
      ...
      affectedCount,
      action: cleanAction,
      message: `已${cleanAction === 'delete' ? '删除' : '归档'} ${affectedCount} 条黑板条目。`
    };
  }
   ...
   * @returns {{ authorSessionId: string, directory: SessionDirectory, callerWorkspace: string }}
   */
  resolveCallerDirectory(agent, ctx) {
  ...
  async executePost(argsOrParams = {}, maybeExec, maybeCtx) {
    ...
    try {
      ...
      const { authorSessionId, directory, callerWorkspace: authorWorkspace } = this.resolveCallerDirectory(callerAgent, ctx);
      const authorTitle = directory.resolveTitle(callerAgent) || 'Session';
      ...
      this.logger.debug?.(`[dsh-call-session] board_post execution failed: ${err?.message || err}`);
      return {
        success: false,
  ...
  async executeList(argsOrParams = {}, maybeExec, maybeCtx) {
    ...
    try {
      const { callerWorkspace } = this.resolveCallerDirectory(exec?.agent, ctx);
      ...
      const res = this.list({
        ...
        callerWorkspace: args?.cross_workspace ? null : callerWorkspace,
      ...
      return {
        ...
        scope: args?.cross_workspace ? 'global' : (callerWorkspace || 'unknown'),
        titlesOnly: res && typeof res.titlesOnly === 'boolean' ? res.titlesOnly : (args?.titles_only !== undefined ? !!args.titles_only : !args?.id),
        posts: postList
      };
    } catch (err) {
      this.logger.debug?.(`[dsh-call-session] board_list execution failed: ${err?.message || err}`);
  ...
  async executeClear(argsOrParams = {}, maybeExec, maybeCtx) {
    ...
    try {
      ...
      this.logger.debug?.(`[dsh-call-session] board_clear execution failed: ${err?.message || err}`);
   ...
   */
  getAuthorReminder(agent, ctx) {
    const { authorSessionId, callerWorkspace } = this.resolveCallerDirectory(agent, ctx);
    if (!authorSessionId || authorSessionId === 'unknown-session') return '';
    const activePosts = this.findActiveByAuthor(authorSessionId, callerWorkspace);
