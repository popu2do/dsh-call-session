...
export class BoardStore {
  ...
  hydratePosts(data) {
    ...
    for (const post of data.posts) {
      ...
      if (isReservedTopic(post.topic)) {
        cleanedDirtyCount++;
        this.logger.warn?.(`[BoardStore] Cleaned dirty post with reserved topic: #${post.id} (topic: ${post.topic})`);
        continue;
      }
    ...
    if (cleanedDirtyCount > 0) {
      this.logger.info?.(`[BoardStore] Hydration complete: filtered ${cleanedDirtyCount} dirty posts containing reserved topics.`);
      this.scheduleFlush();
    }
   ...
   */
  clear({ id, topic, action = 'archive', callerWorkspace, ctx } = {}) {
    if (!id && !topic) {
      ...
      if (post) {
        ...
        if (effectiveCallerWs && postWs && postWs !== effectiveCallerWs) {
          return {
            ...
            affectedCount: 0,
            action: cleanAction,
            message: this.getMessages(ctx).boardClearSuccess(0, cleanAction)
          };
        }
    ...
    return {
      ...
      affectedCount,
      action: cleanAction,
      message: this.getMessages(ctx).boardClearSuccess(affectedCount, cleanAction)
    };
  }
