...
(function () {
  ...
  // ADR-0013: 落点脉冲时序 —— 视口位移补间结束后执行双周期呼吸，严禁几何形变 (ADR-0018)
  var LANDED_PULSE_DELAY_MS = 320;
  var LANDED_PULSE_CYCLES = 2;
  var LANDED_PULSE_CYCLE_MS = 600;
  var LANDED_PULSE_TOTAL_MS = LANDED_PULSE_DELAY_MS + LANDED_PULSE_CYCLES * LANDED_PULSE_CYCLE_MS;
  ...
  var CANVAS_CSS = [
    ...
    '  animation: dshLocatePulse ' + (LANDED_PULSE_CYCLE_MS / 1000) + 's cubic-bezier(0.4, 0, 0.6, 1) ' + (LANDED_PULSE_DELAY_MS / 1000) + 's ' + LANDED_PULSE_CYCLES + ';',
    '}',
    '/* Scoped Light Theme */',
  ...
  function createClientPlugin(requireFn) {
    ...
    function CanvasView(props) {
      ...
      var effectRunCountRef = React ? React.useRef(0) : { current: 0 };
      ...
      if (React && React.useEffect) {
        ...
        React.useEffect(function () {
          ...
          // ADR-0013 §2.10: 外部会话切换仅静默迁移高亮，严禁任何自动取景劫持用户视口
          if (effectRunCountRef.current > 1) userInteractedRef.current = true;
      ...
      return h('div', {
        ...
        h('div', { className: 'dsh-canvas-toolbar' },
          h('div', { className: 'dsh-canvas-toolbar-group' },
            ...
            h('button', { className: 'dsh-canvas-btn', onClick: function () { fitView(true, 'macro'); } }, t('toolbar.fitView')),
            sessionId ? h('button', { className: 'dsh-canvas-btn', onClick: function () { fitView(true, 'locate'); } }, t('toolbar.locateCurrentSession')) : null,
