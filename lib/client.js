...
(function () {
  ...
  var en = {
    ...
    'toolbar.crossWorkspace': 'Cross-Workspace',
  ...
  /**
   * Generates a curved smooth cubic Bezier path string between two coordinates
   */
  function calculateBezierPath(x1, y1, x2, y2, index, total) {
    ...
    if (Math.abs(dx) < 2 && Math.abs(dy) < 2) {
      var pStart = getEllipseIntersection(x1, y1, x1 - 45, y1 - 60, 96, 26);
      var pEnd = getEllipseIntersection(x1, y1, x1 + 45, y1 - 60, 96, 26);
      var c1x = x1 - 50;
      var c1y = y1 - 75;
      var c2x = x1 + 50;
      var c2y = y1 - 75;
      return 'M ' + pStart.x.toFixed(1) + ' ' + pStart.y.toFixed(1) +
    ...
    var c1x = x1 + dx * 0.33 + nx * spread;
    var c1y = y1 + dy * 0.33 + ny * spread;
    var c2x = x1 + dx * 0.67 + nx * spread;
    var c2y = y1 + dy * 0.67 + ny * spread;

    var pStart = getEllipseIntersection(x1, y1, c1x, c1y, 96, 26);
    var pEnd = getEllipseIntersection(x2, y2, c2x, c2y, 96, 26);
  ...
  function computeLayout(workspaces, sessions, posts, currentWorkspace, crossWorkspace, calls) {
    ...
    activeWorkspaces.forEach(function (ws, wsIdx) {
      ...
      var wsSessions = (sessions || []).filter(function (s) {
        ...
        return s.workspace === ws.id || (s.workspace && ws.id && s.workspace.toLowerCase() === ws.id.toLowerCase());
      });
    ...
    if (calls && calls.length) {
      calls.forEach(function (c) {
        var endpoints = [];
        if (c.callerArchived || c.callerOffline || (c.status === 'settled' && c.callerTitle)) {
          endpoints.push({ id: c.callerSessionId, ws: c.callerWorkspace, title: c.callerTitle });
        }
        if (c.targetArchived || c.targetOffline || (c.status === 'settled' && c.targetTitle)) {
          endpoints.push({ id: c.targetSessionId, ws: c.targetWorkspace, title: c.targetTitle });
        }
        endpoints.forEach(function (ep) {
          if (!ep.id) return;
          var sid = String(ep.id);
          var lower = sid.toLowerCase();
          if (nodeIdIndex[sid] || nodeIdIndex[lower] || nodePositions[sid] || nodePositions[lower]) return;

          var targetWs = activeWorkspaces.find(function (w) {
            return ep.ws && (w.id === ep.ws || w.id.toLowerCase() === ep.ws.toLowerCase());
          }) || activeWorkspaces[0];

          var wsBound = workspaceBounds.find(function (wb) { return wb.id === (targetWs ? targetWs.id : null); }) || workspaceBounds[0];
          var wsX = wsBound ? wsBound.x : startX;
          var countInWs = Object.keys(nodePositions).filter(function (k) {
            return nodePositions[k].session && nodePositions[k].session.workspace === (targetWs ? targetWs.id : null);
          }).length;

          var cx = wsX + 130;
          var cy = 214 + countInWs * 72;
          var shortId = computeSessionShortId(sid);
          var virtualSession = {
            id: sid,
            shortId: shortId,
            title: (ep.title && typeof ep.title === 'string' && ep.title.trim()) ? ep.title.trim() : ('Agent ' + shortId),
            workspace: targetWs ? targetWs.id : currentWorkspace,
            state: 'offline',
            status: 'offline',
            isArchived: true,
            isVirtual: true
          ...
          nodePositions[sid] = virtualNode;
          nodeIdIndex[sid] = sid;
          nodeIdIndex[lower] = sid;

          if (wsBound) {
            wsBound.height = Math.max(wsBound.height, cy - wsBound.y + 60);
          }
        });
      });
    }
  ...
  function createClientPlugin(requireFn) {
    ...
    function CanvasView(props) {
      ...
      var defaultCrossWorkspace = props.crossWorkspace !== undefined ? Boolean(props.crossWorkspace) : false;
      ...
      var crossWorkspaceState = React ? React.useState(defaultCrossWorkspace) : [defaultCrossWorkspace, function () {}];
      var crossWorkspace = crossWorkspaceState[0];
      var setCrossWorkspace = crossWorkspaceState[1];

      var fetchSnapshot = function (overrideCrossWs) {
        var seq = ++fetchSeqRef.current;
        var effCrossWs = overrideCrossWs !== undefined ? Boolean(overrideCrossWs) : crossWorkspace;
        ...
        if (typeof props.fetchTelemetry === 'function') {
          props.fetchTelemetry({ crossWorkspace: effCrossWs }).then(function (data) {
            applyData(data);
          }).catch(function () {});
          return;
        }
        if (typeof fetch === 'function') {
          ...
          if (effCrossWs) query.set('crossWorkspace', 'true');
      ...
      if (React && React.useEffect) {
        ...
        React.useEffect(function () {
          ensureStyles();
          fetchSnapshot(crossWorkspace);

          var timer = null;
          var startPolling = function (intervalMs) {
            if (timer) clearInterval(timer);
            timer = setInterval(function () { fetchSnapshot(crossWorkspace); }, intervalMs);
          };
      ...
      var layout = computeLayout(workspaces, sessions, posts, currentWorkspace, crossWorkspace, calls);
      ...
      return h('div', {
          ...
          h('div', { className: 'dsh-canvas-toolbar-group' },
            ...
            h('button', {
              ...
              onClick: function () {
                ...
                var nextWs = !crossWorkspace;
                setCrossWorkspace(nextWs);
                prevChecksumRef.current = '';
                fetchSnapshot(nextWs);
              }
            }, t('toolbar.crossWorkspace')),
        ...
        h('div', { ref: viewportRef, className: 'dsh-canvas-viewport' },
            ...
            blackboardBound ? (function () {
              ...
              return h('g', { id: 'dsh-canvas-blackboard-layer' },
                h('rect', {
                  ...
                  stroke: isBbHovered ? '#38bdf8' : '#475569',
                ...
                var strokeColor = isPostActive ? '#38bdf8' : (isArchived ? '#334155' : (isExpired ? '#1e293b' : '#64748b'));
                var strokeWidth = isPostActive ? '1.8' : (isArchived || isExpired ? '1' : '1.2');
                var strokeDash = isPostActive ? 'none' : (isArchived ? '2 3' : (isExpired ? '1 3' : '4 3'));
                var fillColor = isPostActive ? 'rgba(15, 23, 42, 0.85)' : (isInactive ? 'rgba(15, 23, 42, 0.35)' : 'rgba(30, 41, 59, 0.45)');
                ...
                return h('g', {
                  ...
                  isInactive ? h('g', { className: 'dsh-canvas-post-badge' },
                    h('rect', {
                      ...
                      ry: 3,
                      fill: '#334155',
                      stroke: isArchived ? '#475569' : '#334155',
                      strokeWidth: '0.8'
                    }),
                    h('text', {
              ...
              workspaceBounds.map(function (ws) {
                ...
                return h('g', {
                  ...
                  h('rect', {
                    ...
                    stroke: isWsHovered ? '#60a5fa' : (ws.isCurrent ? '#64748b' : '#334155'),
            ...
            h('g', { id: 'dsh-canvas-author-edges' },
              posts.map(function (post) {
                ...
                return h('path', {
                  ...
                  strokeDasharray: isHighlighted ? '4 2' : '4 3',
              ...
              Object.keys(nodePositions).map(function (sid) {
                ...
                return h('g', {
                  ...
                  h('text', {
                    ...
                    x: -52,
                    y: 12,
                    fill: isCurrentSession ? '#38bdf8' : '#f1f5f9',
                    fontSize: '12',
                    fontFamily: 'var(--dsh-font-sans)',
          ...
          hoveredEntity ? (function () {
            ...
            // 实体吸附定位计算
            var tipHeightEst = kind === 'session' ? 105 : (kind === 'call' ? 95 : 100);
            ...
            if (kind === 'session') {
              ...
              if (sNode) {
                ...
                tipX = screenX - 140; // Tooltip 居中 (max-width 280px)
                // 优先置于实体上方 8px (椭圆 ry=26)；若上方超出视口则置于下方 8px
                if (screenY - 26 * zoom - tipHeightEst - 8 >= 10) {
                  tipY = screenY - 26 * zoom - 8 - tipHeightEst;
                } else {
                  tipY = screenY + 26 * zoom + 8;
                }
              } else {
            ...
            if (kind === 'session') {
              ...
              return h('div', {
                ...
                h('div', { className: 'dsh-canvas-tooltip-header' },
                  ...
                  h('span', { style: { color: statusColor } }, sessionStatusText)
                ),
                h('div', { style: { fontWeight: '500', color: '#f1f5f9', marginBottom: '4px' } }, ent.title || ('Agent ' + (ent.shortId || computeSessionShortId(ent.id)))),
                ...
                h('div', { className: 'dsh-canvas-tooltip-row' },
                  ...
                  h('span', { style: { fontFamily: 'var(--dsh-font-mono)' } }, ((ent.stats && ent.stats.inboundCalls) || 0) + ' / ' + ((ent.stats && ent.stats.outboundCalls) || 0))
                )
              );
            } else if (kind === 'call') {
              return h('div', {
