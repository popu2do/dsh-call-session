...
(function () {
  ...
  // ADR-0013: 视口位移补间时长是过渡样式、过渡态计时与落点脉冲延迟的唯一真源
  var VIEWPORT_TWEEN_MS = 320;
  var VIEWPORT_TWEEN_SEC = VIEWPORT_TWEEN_MS / 1000;
  // 落点脉冲在补间结束后执行双周期呼吸，严禁几何形变 (ADR-0018)
  var LANDED_PULSE_CYCLES = 2;
  var LANDED_PULSE_CYCLE_MS = 600;
  var LANDED_PULSE_TOTAL_MS = VIEWPORT_TWEEN_MS + LANDED_PULSE_CYCLES * LANDED_PULSE_CYCLE_MS;
  ...
  var en = {
    ...
    'blackboard.active': 'active',
    'blackboard.total': 'total',
  ...
  var CANVAS_CSS = [
    ...
    '}',
    '@keyframes dshLocatePulse {',
    '  0% { stroke-opacity: 1; }',
    '  50% { stroke-opacity: 0.25; }',
    '  100% { stroke-opacity: 1; }',
    '}',
    '.dsh-canvas-container .dsh-canvas-node-locating {',
    '  animation: dshLocatePulse ' + (LANDED_PULSE_CYCLE_MS / 1000) + 's cubic-bezier(0.4, 0, 0.6, 1) ' + VIEWPORT_TWEEN_SEC + 's ' + LANDED_PULSE_CYCLES + ';',
    '}',
    '/* Scoped Light Theme */',
    ...
    '  }',
    '}',
    '/* ADR-0014: Viewport Width-Handle Suppression */',
    'body:has(.dsh-canvas-container) [data-width-handle] {',
  ...
  /**
   * Computes string pixel width based on rendered character metrics
   */
  function measureTextWidth(text, charWidthAscii, charWidthWide) {
    if (!text) return 0;
    var str = String(text);
  ...
  function truncateTextByWidth(text, maxWidth, charWidthAscii, charWidthWide) {
    ...
    var asciiW = typeof charWidthAscii === 'number' ? charWidthAscii : 7.2;
    var wideW = typeof charWidthWide === 'number' ? charWidthWide : 12.5;
    var totalW = measureTextWidth(str, asciiW, wideW);
  ...
  var GUTTER_CONFIG = Object.freeze({
    ...
    CHANNEL_MAX_OFFSET: 126,
    CHANNEL_HOP_SPREAD: 4,
    CHANNEL_MAX_SPREAD: 18,
    CHANNEL_JITTER_STEP: 2,
  ...
  function calculateBezierPath(x1, y1, x2, y2, index) {
    ...
    if (Math.abs(dx) < 2) {
      // ADR-0019 通道分流走线：同工作区列内上下调用执行上下行双通道隔离，避免横穿列中轴
      var hopCount = Math.max(1, Math.round(Math.abs(dy) / GUTTER_CONFIG.SESSION_SLOT_HEIGHT));
      var dynamicSpread = Math.min((hopCount - 1) * GUTTER_CONFIG.CHANNEL_HOP_SPREAD, GUTTER_CONFIG.CHANNEL_MAX_SPREAD) + (Math.abs(curveIndex) % 2) * GUTTER_CONFIG.CHANNEL_JITTER_STEP;
      var concentricOffset = Math.min(GUTTER_CONFIG.CHANNEL_MIN_OFFSET + dynamicSpread, GUTTER_CONFIG.CHANNEL_MAX_OFFSET);
      ...
      // 跨列反向调用 (|dx| >= 200，跨列物理最小间距为 296px)：通过顶层走线通道 (y <= 130) 横跨避障，避免穿透卡片
  ...
  /**
   * Generates two-segment Bezier path string for author-blackboard edges (ADR-0014, ADR-0019)
   */
  function calculateAuthorEdgePath(sourceNode, targetPost, edgeIndex) {
    var edgeCoords = computePostEdgeCoordinates(sourceNode, targetPost, edgeIndex);
    return 'M ' + edgeCoords.startX.toFixed(1) + ' ' + edgeCoords.startY.toFixed(1) +
  ...
  /**
   * Generates single-segment Bezier path string for context-post edges (ADR-0014, ADR-0019)
   */
  function calculateContextEdgePath(sourceNode, targetPost, edgeIndex) {
    var edgeCoords = computePostEdgeCoordinates(sourceNode, targetPost, edgeIndex);
    return 'M ' + edgeCoords.startX.toFixed(1) + ' ' + edgeCoords.startY.toFixed(1) +
      ' C ' + edgeCoords.gutterX.toFixed(1) + ' ' + edgeCoords.startY.toFixed(1) + ', ' +
      edgeCoords.gutterX.toFixed(1) + ' ' + edgeCoords.gutterY.toFixed(1) + ', ' +
  ...
  function computeLayout(workspaces, sessions, posts, currentWorkspace, crossWorkspace, calls) {
    ...
    // ADR-0013: 当前工作区始终固定排在最左侧首列 (index 0) 并标注 isCurrent（显式路径优先，未匹配时自动补齐，避免陈旧 isCurrent 覆盖）
    if (currentWorkspace) {
  ...
  /**
   * @typedef {{ minX: number, minY: number, maxX: number, maxY: number }} CanvasBounds 内容外接矩形
   * @typedef {{ zoom: number, panX: number, panY: number }} ViewportFraming 视口取景结果
   * @typedef {{ mode?: 'macro' | 'locate', viewportWidth?: number, viewportHeight?: number,
   *             bounds?: CanvasBounds, anchor?: { x: number, y: number } | null,
   *             currentZoom?: number }} ViewportFramingOptions 视口取景选项
   */

  /**
   * ADR-0013: 看板视口取景引擎
   * 纯函数，不读取组件 refs 与状态。宏观取景与夹紧定位共用同一套边界几何。
   */
  ...
  // 空拓扑下的兜底外接矩形与节点半尺寸，供取景引擎与内容边界共用
  var CANVAS_FALLBACK_BOUNDS = { minX: 40, minY: 24, maxX: 1120, maxY: 650 };
  var SESSION_NODE_HALF_WIDTH = 96;
  var SESSION_NODE_HALF_HEIGHT = 26;

  /**
   * @param {ViewportFramingOptions} options
   * @returns {ViewportFraming}
   */
  function computeViewportFraming(options) {
    var opts = options || {};
    var vw = Number(opts.viewportWidth) > 0 ? Number(opts.viewportWidth) : 1000;
    var vh = Number(opts.viewportHeight) > 0 ? Number(opts.viewportHeight) : 656;
    var bounds = opts.bounds || {};
    var anchor = opts.anchor || null;
    var isLocate = opts.mode === 'locate';
    var padding = VIEWPORT_FRAMING.PADDING;

    var minX = isFinite(bounds.minX) ? bounds.minX : CANVAS_FALLBACK_BOUNDS.minX;
    var minY = isFinite(bounds.minY) ? bounds.minY : CANVAS_FALLBACK_BOUNDS.minY;
    var maxX = isFinite(bounds.maxX) ? bounds.maxX : CANVAS_FALLBACK_BOUNDS.maxX;
    var maxY = isFinite(bounds.maxY) ? bounds.maxY : CANVAS_FALLBACK_BOUNDS.maxY;

    var contentW = Math.max(maxX - minX, 50);
    var contentH = Math.max(maxY - minY, 50);
    ...
    if (isLocate && anchor) {
      // 定位模式：维持用户视距，双向夹紧至可读区间，禁止粗暴重置为 1.0
      var baseZoom = Number(opts.currentZoom);
      if (!isFinite(baseZoom) || baseZoom <= 0) baseZoom = 1;
      zoom = Math.min(Math.max(baseZoom, VIEWPORT_FRAMING.LOCATE_MIN_ZOOM), VIEWPORT_FRAMING.LOCATE_MAX_ZOOM);

      // 水平夹紧居中：内容整体容纳于视口时居中，超出时首列左边缘对齐安全边距，避免左侧空白断层
      var centeredPanX = (vw - contentW * zoom) / 2 - minX * zoom;
      panX = contentW * zoom <= vw ? centeredPanX : padding - minX * zoom;

      // 垂直边界约束：目标会话居中为基线；列高适配视口时受顶部黑板与列底双向夹紧。
      var anchorY = Number(anchor.y) || 0;
      var centeredPanY = (vh / 2) - anchorY * zoom;
      var topBoundPanY = VIEWPORT_FRAMING.TOP_MARGIN - minY * zoom;
      var fitsVertically = contentH * zoom <= vh - VIEWPORT_FRAMING.TOP_MARGIN - VIEWPORT_FRAMING.BOTTOM_MARGIN;
      if (fitsVertically) {
        var bottomBoundPanY = vh - VIEWPORT_FRAMING.BOTTOM_MARGIN - maxY * zoom;
        panY = Math.min(Math.max(centeredPanY, topBoundPanY), bottomBoundPanY);
      } else {
        // 列高超出视口：节点可见性优先；仅当保留顶部黑板不会推出目标会话时才抬升 panY
        var topAnchoredPanY = Math.max(centeredPanY, topBoundPanY);
        var anchorScreenY = anchorY * zoom + topAnchoredPanY;
        panY = (anchorScreenY >= VIEWPORT_FRAMING.TOP_MARGIN && anchorScreenY <= vh - VIEWPORT_FRAMING.BOTTOM_MARGIN)
          ? topAnchoredPanY
          : centeredPanY;
      }
    } else {
      // 宏观取景：完整框定公共黑板与全部工作区分组列
      var availW = Math.max(vw - padding * 2, 100);
      var availH = Math.max(vh - padding * 2, 100);
      zoom = Math.min(Math.max(Math.min(availW / contentW, availH / contentH), VIEWPORT_FRAMING.MACRO_MIN_ZOOM), VIEWPORT_FRAMING.MACRO_MAX_ZOOM);
  ...
  function createClientPlugin(requireFn) {
    ...
    function CanvasDrawer(props) {
      ...
      var badgeLabel = typeof t === 'function' ? t('drawer.readonlyBadge') : '';
    ...
    function CanvasView(props) {
      ...
      var toggleTheme = function () {
        ...
        // ADR-0014: 作用域隔离，不修改宿主外部 document.body 属性
        setIsDark(nextDark);
      };
      ...
      var initialViewport = (function () {
        ...
        var framing = computeViewportFraming({
          viewportWidth: vw,
          viewportHeight: vh,
          bounds: CANVAS_FALLBACK_BOUNDS
        });
        return {
          pan: { x: Math.round(framing.panX), y: Math.round(framing.panY) },
      ...
      var lastObservedHeightRef = React ? React.useRef(0) : { current: 0 };
      ...
      var pulseTimerRef = React ? React.useRef(null) : { current: null };
      var pulsingSessionIdState = React ? React.useState(null) : [null, function () {}];
      var pulsingSessionId = pulsingSessionIdState[0];
      var setPulsingSessionId = pulsingSessionIdState[1];
      ...
      if (React && React.useEffect) {
          ...
          if (hasData && !initialFitDoneRef.current) {
            ...
            if (rawW > 0 && rawH > 0) {
              ...
              if (typeof fitViewRef.current === 'function') {
                fitViewRef.current(false, 'macro');
              }
            }
          }
        }, [telemetry]);
        ...
        React.useEffect(function () {
          ...
          if (RO) {
            ...
            if (targetEl) {
              // ResizeObserver 在 observe() 时投递一次与当前布局同尺寸的初始观测，那不是尺寸变化。
              // 以「尺寸是否真的改变」判定而非「是否首次回调」，宿主切换会话重建观察器时
              // 严禁借此劫持用户视口，且与初始观测合并的真实尺寸变化仍能触发复取景 (ADR-0013 §2.1.1)
              resizeObserver = new RO(function (entries) {
                for (var i = 0; i < entries.length; i++) {
                  ...
                  if (width > 0 && height > 0) {
                    var wasZero = lastObservedWidthRef.current <= 0 || lastObservedHeightRef.current <= 0;
                    var sizeChanged = Math.abs(width - lastObservedWidthRef.current) > 0.5 || Math.abs(height - lastObservedHeightRef.current) > 0.5;
                    lastObservedWidthRef.current = width;
                    lastObservedHeightRef.current = height;
                    if (!initialFitDoneRef.current || wasZero || (!userInteractedRef.current && sizeChanged)) {
      ...
      /**
       * ADR-0013: 聚合公共黑板、工作区分组列与全部会话节点的内容外接矩形
       * @returns {CanvasBounds}
       */
      var getContentBounds = function () {
        ...
        Object.keys(nodePositions).forEach(function (sid) {
          ...
          minX = Math.min(minX, node.x - SESSION_NODE_HALF_WIDTH);
          minY = Math.min(minY, node.y - SESSION_NODE_HALF_HEIGHT);
          maxX = Math.max(maxX, node.x + SESSION_NODE_HALF_WIDTH);
          maxY = Math.max(maxY, node.y + SESSION_NODE_HALF_HEIGHT);
        });
        ...
        if (!isFinite(minX) || !isFinite(maxX)) {
          return {
            minX: CANVAS_FALLBACK_BOUNDS.minX,
            minY: CANVAS_FALLBACK_BOUNDS.minY,
            maxX: CANVAS_FALLBACK_BOUNDS.maxX,
            maxY: CANVAS_FALLBACK_BOUNDS.maxY
          };
        }
      ...
      // ADR-0013: 落点脉冲 —— 位移补间结束后触发双周期呼吸，严禁几何形变 (ADR-0018)
      var triggerLandedPulse = function () {
        if (pulseTimerRef.current) clearTimeout(pulseTimerRef.current);
        setPulsingSessionId(sessionId);
        pulseTimerRef.current = setTimeout(function () {
          pulseTimerRef.current = null;
          setPulsingSessionId(null);
        }, LANDED_PULSE_TOTAL_MS);
      };

      // 适配内容边界并执行自适应居中
      var fitView = function (smooth, mode) {
        var container = containerRef.current;
        var viewport = viewportRef.current || (container && container.querySelector ? container.querySelector('.dsh-canvas-viewport') : null);
        var rawW = (viewport && viewport.clientWidth > 0) ? viewport.clientWidth : ((container && container.clientWidth > 0) ? container.clientWidth : 0);
        var rawH = (viewport && viewport.clientHeight > 0) ? viewport.clientHeight : ((container && container.clientHeight > 0) ? Math.max(100, container.clientHeight - 44) : 0);
        ...
        var framing = computeViewportFraming({
          mode: currentNode ? 'locate' : 'macro',
          viewportWidth: vw,
          viewportHeight: vh,
          bounds: getContentBounds(),
          anchor: currentNode ? { x: currentNode.x, y: currentNode.y } : null,
          currentZoom: zoom
        });
        ...
        setPan({ x: Math.round(framing.panX), y: Math.round(framing.panY) });

        if (currentNode) triggerLandedPulse();
      ...
      return h('div', {
        ...
        h('div', { className: 'dsh-canvas-toolbar' },
          h('div', { className: 'dsh-canvas-toolbar-group' },
            ...
            h('button', { className: 'dsh-canvas-btn', onClick: function () { userInteractedRef.current = false; fitView(true, 'macro'); } }, t('toolbar.fitView')),
            sessionId ? h('button', { className: 'dsh-canvas-btn', onClick: function () { userInteractedRef.current = true; fitView(true, 'locate'); } }, t('toolbar.locateCurrentSession')) : null,
        ...
        h('div', { ref: viewportRef, className: 'dsh-canvas-viewport' },
          h('svg', {
            ...
            style: {
              ...
              transition: isTransitioning ? ('transform ' + VIEWPORT_TWEEN_SEC + 's cubic-bezier(0.25, 1, 0.5, 1)') : 'none',
            ...
            blackboardBound ? (function () {
              ...
              return h('g', { id: 'dsh-canvas-blackboard-layer' },
                ...
                (function () {
                  ...
                  var activeLabel = t('blackboard.active');
                  var totalLabel = t('blackboard.total');
                  var countText = activePostsCount !== posts.length
                    ? '(' + activePostsCount + ' ' + activeLabel + ' / ' + posts.length + ' ' + totalLabel + ')'
                    : '(' + activePostsCount + ')';
                  var countX = Math.round(measureTextWidth(hubTitle, 7.0, 12.5) + 8);
                ...
                return h('g', {
                  ...
                  h('g', { transform: 'translate(12, 10)' },
                    ...
                    h('text', {
                      ...
                      fontSize: '12',
                      fontWeight: '600'
                    }, truncateTextByWidth(ws.name || 'Workspace', 160) + (ws.isCurrent ? (' [' + t('workspace.current') + ']') : ''))
                  )
                );
              })
            ),
            ...
            h('g', { id: 'dsh-canvas-author-edges' },
              posts.map(function (post, pIdx) {
                ...
                var pathD = calculateAuthorEdgePath(source, targetPost, pIdx);
            ...
            h('g', { id: 'dsh-canvas-context-edges' },
              calls.map(function (call, cIdx) {
                ...
                return call.contextPostIds.map(function (pid, pidIdx) {
                  ...
                  // ADR-0014: 右侧走线通道引出，经顶部外围通道汇入黑板条目，避免斜切卡片
                  var slotIndex = (cIdx * 7 + pidIdx) % 20;
                  var pathD = calculateContextEdgePath(callerNode, pPos, slotIndex);
            ...
            h('g', { id: 'dsh-canvas-call-edges' },
              calls.map(function (call, cIdx) {
                ...
                // ADR-0019 常态按需显影：未交互时已结算历史调用隐藏，保持视图清晰
            ...
            h('g', { id: 'dsh-canvas-nodes' },
              Object.keys(nodePositions).map(function (sid) {
                ...
                var currentTag = isCurrentSession ? ('[' + t('session.current') + ']') : '';
                var badgeWidth = isCurrentSession ? Math.round(measureTextWidth(currentTag, 5.5, 9.5) + 10) : 0;
                var currentBadgeStart = 18;
                var titleOriginX = -52;
                var titleMaxWidth = isCurrentSession ? Math.max(48, currentBadgeStart - titleOriginX - 8) : (hasCallBadge ? 105 : 140);
                var statsX = isCurrentSession ? Math.min(84, Math.max(70, currentBadgeStart + badgeWidth + 10)) : 70;
                var isLocatingPulse = Boolean(pulsingSessionId) && isMatchingSession(sid, pulsingSessionId, shortId);
                ...
                return h('g', {
                  ...
                  h('ellipse', {
                    className: isLocatingPulse ? 'dsh-canvas-node-locating' : '',
                    cx: 0,
                    cy: 0,
                    rx: SESSION_NODE_HALF_WIDTH,
                    ry: SESSION_NODE_HALF_HEIGHT,
                  ...
                  hasCallBadge ? h('g', { transform: 'translate(' + statsX + ', 0)' },
          ...
          hoveredEntity ? (function () {
            ...
            if (kind === 'session') {
              ...
              if (sNode) {
                ...
                // 优先置于卡片下方净间隙 (椭圆 ry=26)，避免遮挡上方卡片文字与左右端口
    ...
    return {
      ...
      computeViewportFraming: computeViewportFraming,
      getEllipseIntersection: getEllipseIntersection,
      computePostEdgeCoordinates: computePostEdgeCoordinates,
      calculateAuthorEdgePath: calculateAuthorEdgePath,
      calculateContextEdgePath: calculateContextEdgePath,
      GUTTER_CONFIG: GUTTER_CONFIG,
      VIEWPORT_FRAMING: VIEWPORT_FRAMING,
      CANVAS_FALLBACK_BOUNDS: CANVAS_FALLBACK_BOUNDS,
      SESSION_NODE_HALF_WIDTH: SESSION_NODE_HALF_WIDTH,
      SESSION_NODE_HALF_HEIGHT: SESSION_NODE_HALF_HEIGHT,
