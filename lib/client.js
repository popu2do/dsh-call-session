...
(function () {
  ...
  function calculateBezierPath(x1, y1, x2, y2, index) {
    ...
    if (Math.abs(dx) < 2) {
      // 同工作区列内上下调用：右出左进外侧绕行，控制点严格收敛在左右走线通道内 [96, 130]
      controlPoint1X = x1 + intraOffset;
      controlPoint2X = x2 - intraOffset;

      // 跨层非相邻节点 (|dy| > 80)：在右侧走线通道纵向直达目标外围空隙高度，绝不横穿中间会话卡片
      if (Math.abs(dy) > 80) {
        var verticalGapY = y2 - (dy > 0 ? 36 : -36);
        var startNodePoint = getEllipseIntersection(x1, y1, controlPoint1X, y1 + (dy >= 0 ? 25 : -25), GUTTER_CONFIG.NODE_RX, GUTTER_CONFIG.NODE_RY);
        var endNodePoint = getEllipseIntersection(x2, y2, controlPoint2X, y2 - (dy >= 0 ? 25 : -25), GUTTER_CONFIG.NODE_RX, GUTTER_CONFIG.NODE_RY);
        return 'M ' + startNodePoint.x.toFixed(1) + ' ' + startNodePoint.y.toFixed(1) +
               ' C ' + controlPoint1X.toFixed(1) + ' ' + (y1 + (dy >= 0 ? 25 : -25)).toFixed(1) + ', ' +
               controlPoint1X.toFixed(1) + ' ' + (verticalGapY - (dy >= 0 ? 15 : -15)).toFixed(1) + ', ' +
               controlPoint1X.toFixed(1) + ' ' + verticalGapY.toFixed(1) +
               ' C ' + controlPoint1X.toFixed(1) + ' ' + (verticalGapY + (dy >= 0 ? 15 : -15)).toFixed(1) + ', ' +
               controlPoint2X.toFixed(1) + ' ' + (y2 - (dy >= 0 ? 25 : -25)).toFixed(1) + ', ' +
               endNodePoint.x.toFixed(1) + ' ' + endNodePoint.y.toFixed(1);
      }

      var verticalLead = Math.max(Math.abs(dy) * 0.35, 25);
      controlPoint1Y = y1 + (dy >= 0 ? verticalLead : -verticalLead);
      controlPoint2Y = y2 + (dy >= 0 ? -verticalLead : verticalLead);
    } else if (dx > 0) {
  ...
  function computePostEdgeCoordinates(sourceNode, targetPost) {
    var startY = sourceNode.y;
    return {
      startX: sourceNode.x + GUTTER_CONFIG.NODE_RX,
      startY: startY,
      gutterX: sourceNode.x + GUTTER_CONFIG.GUTTER_OFFSET,
      gutterY: Math.min(startY - GUTTER_CONFIG.POST_LEAD_Y, GUTTER_CONFIG.POST_CEILING_Y),
  ...
  function computeLayout(workspaces, sessions, posts, currentWorkspace, crossWorkspace, calls) {
    ...
    activeWorkspaces.forEach(function (workspace, activeWorkspaceIndex) {
      ...
      workspaceSessions.forEach(function (session, sessionIndex) {
        ...
        var centerY = 214 + sessionIndex * 72;
    ...
    if (calls && calls.length) {
      calls.forEach(function (callRecord) {
        ...
        endpoints.forEach(function (endpoint) {
          ...
          var centerY = 214 + sessionCountInWorkspace * 72;
    ...
    var blackboardWidthFromPosts = 56 + postList.length * (180 + 16) + 40;
    var blackboardWidth = Math.max(1080, blackboardWidthFromWorkspaces, blackboardWidthFromPosts);
    ...
    postList.forEach(function (post, postIndex) {
          ...
          h('svg', {
            ...
            h('g', { id: 'dsh-canvas-author-edges' },
              posts.map(function (post) {
                ...
                // ADR-0014: Right-Exit Gutter Routing
                var edgeCoords = computePostEdgeCoordinates(source, targetPost);
                ...
                return h('path', {
                  ...
                  className: isDimmed ? 'dsh-canvas-dimmed' : (isHighlighted ? 'dsh-pulse-edge' : 'dsh-canvas-highlighted'),
            ...
            h('g', { id: 'dsh-canvas-context-edges' },
              calls.map(function (call, cIdx) {
                ...
                return call.contextPostIds.map(function (pid) {
                  ...
                  var edgeCoords = computePostEdgeCoordinates(callerNode, pPos);
              ...
              calls.map(function (call, cIdx) {
                ...
                return h('g', {
                  ...
                  h('path', {
                    ...
                    markerEnd: markerUrl,
                    className: (isCallHovered || isConnected) ? (decay.isFlowing ? 'dsh-flow-edge' : 'dsh-pulse-edge') : decay.className,
