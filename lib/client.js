...
(function () {
  ...
  function createClientPlugin(requireFn) {
    ...
    function CanvasView(props) {
      ...
      var crossWorkspace = props.crossWorkspace !== undefined ? Boolean(props.crossWorkspace) : true;
      ...
      var fetchSnapshot = function () {
        ...
        var applyData = function (data) {
          ...
          setTelemetry(data);
          if (!initialFitDoneRef.current && ((data.sessions && data.sessions.length > 0) || (data.posts && data.posts.length > 0))) {
            initialFitDoneRef.current = true;
            fitTimeoutRef.current = setTimeout(function () {
              if (typeof fitView === 'function') fitView(true);
            }, 60);
          }
        };

        if (typeof props.fetchTelemetry === 'function') {
          props.fetchTelemetry({ crossWorkspace: true }).then(function (data) {
            applyData(data);
          }).catch(function () {});
          return;
        }
      ...
      return h('div', {
        ...
        h('div', { className: 'dsh-canvas-toolbar' },
          h('div', { className: 'dsh-canvas-toolbar-group' },
            ...
            h('button', { className: 'dsh-canvas-btn', onClick: function () { fitView(true); } }, t('toolbar.fitView')),
          ...
          h('svg', {
            ...
            blackboardBound ? (function () {
              ...
              return h('g', { id: 'dsh-canvas-blackboard-layer' },
                ...
                var fillColor = isPostActive ? 'rgba(15, 23, 42, 0.85)' : 'rgba(30, 41, 59, 0.45)';
            ...
            h('g', { id: 'dsh-canvas-nodes' },
              Object.keys(nodePositions).map(function (sid) {
                ...
                var strokeColor = isHighlighted
                  ? (isRunning ? '#4ade80' : '#93c5fd')
                  : (isRunning ? '#22c55e' : '#64748b');
                var strokeWidth = isHighlighted ? '2.2' : (isRunning ? '1.5' : '1.2');
                var strokeDash = isHighlighted ? 'none' : (isRunning ? '5 3' : '4 4');
                ...
                return h('g', {
                  ...
                  h('ellipse', {
                    ...
                    strokeDasharray: strokeDash,
                    style: {
                      filter: isHighlighted ? (isRunning ? 'drop-shadow(0 0 8px rgba(74, 222, 128, 0.45))' : 'drop-shadow(0 0 8px rgba(147, 197, 253, 0.45))') : 'none'
                    }
                  }),
                  h('circle', {
                    cx: -68,
                  ...
                  h('text', {
                    ...
                    fontSize: '12',
                    fontFamily: 'var(--dsh-font-sans)'
                  }, truncateTextByWidth(session.title || ('Agent ' + shortId), 105)),
